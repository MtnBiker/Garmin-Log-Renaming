...
baseFolder = "/Users/gscar/Dropbox/   Garmin gpx daily logs/"
folderDownload = baseFolder + "2013 Massaged/"
puts "folderDownload: #{folderDownload}"
folderMassaged = "/Users/gscar/Dropbox/   Garmin gpx daily logs/2013 Massaged/"
...
oldTEMPfiles = "/Users/gscar/Dropbox/   Garmin gpx daily logs/old TEMP files/" # for files created on day of download which may not be complete
# 
...
def loc(arr)
  ...
  puts "places_nearby: #{places_nearby}. places_nearby places_nearby.first.name: #{places_nearby.first.name}. places_nearby.first.country_name: #{places_nearby.first.country_name}"
...
puts "\nProcessing (Massaged) files in #{folderMassaged}\n"
#  fc = file contents
#  fr = file revised
#  ts = time shift (hours)
#  ln = line number
...
if fromWhichFolder == folderOnGarmin
  ...
  Find.find(folderOnGarmin) do |fx|
    ...
    if File.file?(fx) # the directory we're looking in is added to the fx list, so have to skip it
      ...
      fnew = "/Users/gscar/Dropbox/   Garmin gpx daily logs/#{yearFile} Download/#{File.basename(fx)}"
      if !File.exists?(fnew) # this goes through all the files on the Garmin which may be a bit slow as the card fills up. If too slow fix to stop after reaching first duplicate
        ...
        if today!=File.basename(fx, ".*") # don't want to copy today in case I'm downloading before the day is done which I might do to see exercise for the day. See else below as this is changed
          puts "\"#{fx}\" copied to \"#{fnew}\". Line ~107"
...
`diskutil unmountDisk /Volumes/GARMIN/` # Unmounting Garmin. Need to check that this is not too soon. But want to do it as soon as Garmin not needed.Unmounts the drive from Mac, but the GPS does not notice it which it does if unmount from Finder
# The following works in AppleScript, i.e. unmount Garmin so that Garmin know it, but cannot get Appscript now to try it
...
#   end
# end
#  my first guess, but could not load appscript
# Appscript.app.by_name("Finder").disk.eject("GARMIN")

puts "\n Garmin unmounted. Now starting processing of gpx files in #{fromWhichFolder}.\n\n"
    ...
    if File.exist?(fx)  # checking if file to be processed exists. Probably not needed now as only working with a list of existing files
      ...
      while ln<alength
        if arr[ln] =~ /<name>ACTIVE LOG[0-9]+<\/name>/
          ...
          puts time # Sun Jun 09 19:09:52 UTC 2013
         ...
         puts "fx: #{fx}. ln: #{ln}. datetime: #{datetime}. Time.new(datetime) #{datetime}. "
         puts "alatlon: #{alatlon}"
      ...
      # Timezone
          ...
          puts "211. #{File.basename(fx)}. timeshifted: #{timeshifted}. timeshifted4name: #{timeshifted4name}"
...
=end
      ...
      fr = arr.join
      # write back file
      puts "\n238. #{File.basename(fx)}. timeshifted: #{timeshifted}. timeshifted4name: #{timeshifted4name}"
