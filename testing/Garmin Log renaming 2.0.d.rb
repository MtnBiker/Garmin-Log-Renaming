#!/usr/bin/env ruby
require "date"
# require 'parsedate' # was ruby 1.8, Date handles most of the needs. NOT SURE THIS IS DEALT WITH YET AS HAVEN'T GOTTEN PAST GEONAMES PROBLEM. SEE BELOW
require 'find'
require 'fileutils'
require 'geocoder' # try when figure out how to add gems to iMac
require 'geonames'# both geonames and geocoder do similar things, but not sure which is better. Geonames is being used for time zones which geocoder doesn't have
require 'Appscript'

# PROBLEMS WITH 1.9 AND 2.0. geonames comes up empty. No errors just no result. I HAVE TESTS IN THIS FOLDER AND IN TEST FOLDERS IN RUBY FOLDER ON IMAC. Also having problem installing gems on 1.8.7: Seemingly a confusion between system and installed 1.8.7, but neither seems to work. 2013.07.12. ALSO A PROBLEM NOTED ON LINE 17
...
=end
# .d updated to ruby 1.9 NOT WORKING BECAUSE Find.find going in a different order
...
def loc(arr)
  ...
  # puts "Error probably due to different geonames gem"
  # puts "{places_nearby.first.name: #{places_nearby.first.name}, #{places_nearby.first.country_name}"  
    ...
    if File.exist?(fx)  # checking if file to be processed exists. Probably not needed now as only working with a list of existing files
      ...
      while ln<alength
        if arr[ln] =~ /<name>ACTIVE LOG[0-9]+<\/name>/
          ...
          datetime = $1 
          # following commented out lines from version g and not needed but keep until go to standard time to make sure working (and I am not sure it is)
          # tp = ParseDate.parsedate datetime # year 0, month 1, day 2, hour 3, min. 4, sec. 5, tz 6, weekday 7 # v1.8.7
          # tp = DateTime.parse datetime # year 0, month 1, day 2, hour 3, min. 4, sec. 5, tz 6, weekday 7
         #  puts tp
         #  puts "tp: #{tp}. #{tp[2]}"
         ...
         #    isdst = "ST"
         #  end # Time.local
         puts "fx: #{fx}. ln: #{ln}. datetime: #{datetime}. Time.new(datetime) #{datetime}. Time.new(datetime).dst?: #{Time.new(datetime).dst?}"
          if Time.new(datetime).dst?
            isdst = "DST"
          else
            isdst = "ST"
          end # Time.local
